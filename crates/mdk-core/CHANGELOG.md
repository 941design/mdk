...
- **Multi-device leaf helpers**: When `create_key_package_for_event_with_options` is called with an explicit `d_tag`, the slot identifier is now embedded into the leaf node via the MLS `application_id` extension so it survives the join into a group and self-update rotations. New `MDK::get_group_leaves(group_id) -> Vec<GroupLeafInfo>` enumerates every occupied leaf — including multiple leaves owned by the same Nostr pubkey — exposing `leaf_index`, `pubkey`, `slot`, and `is_own_leaf`. New `MDK::remove_leaves(group_id, &[u32])` removes specific leaves by index without affecting sibling leaves owned by the same pubkey, and only strips a removed leaf's owner from the admin list when none of their other leaves survive. Bare `create_key_package_for_event` callers (no explicit `d_tag`) keep the byte-identical legacy KeyPackage wire format. <!-- TODO: append ([#NNN](https://github.com/marmot-protocol/mdk/pull/NNN)) once PR opened -->
